      ...
      <div className="container mx-auto lg:px-4 md:px-6">
        <nav className="lg:flex md:flex  items-center justify-between hidden">
          ...
          {username && (
            <>
              <div className="uppercase space-x-4">
                <Link className={`"text-zinc-900 hover:text-zinc-700 ${location.pathname === '/home' ? navActive : navInactive
                  }`} to={'/home'}>
                  Home
                </Link>
                <Link className={`"text-zinc-900 hover:text-zinc-700 ${location.pathname === '/globalApi' ? navActive : navInactive
                  }`} to={'globalApi'} >
                  Global Recipes
                </Link>
              </div>
              <div className="uppercase space-x-4">
                {location.pathname !== '/profile' && (
                  <Link className={`text-zinc-900 hover:text-zinc-700 ${location.pathname === '/write' ? active : inactive} `} to={'write'} >
                    Create
                  </Link>
                )}
                <Link className={`"text-zinc-900 hover:text-zinc-700  ${location.pathname === '/profile' ? navActive : navInactive
                  }`} to={'profile'} >
                  Profile
                </Link>
              </div>
          ...
          {!username && (
            <>
              <div className="uppercase space-x-4">
                <Link className={`"text-zinc-900 hover:text-zinc-700 ${location.pathname === '/' ? navActive : navInactive
                  }`} to={'/'}>
                  Home
                </Link>
              </div>
          ...
          {username && (
            <>
              ...
              <div className="uppercase space-x-4  flex items-center">
                {location.pathname !== '/profile' && (
                  <Link className={`text-zinc-900 hover:text-zinc-700 rounded-full ${location.pathname === '/write' ? active : inactive} `} to={'write'} >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                  </Link>
                )}
                <Link className={`text-zinc-900 hover:text-zinc-700 text-3xl rounded-full bg-blue-50  ${location.pathname === '/profile' ? profileActive : navInactive
                  }`} to={'profile'} >
                  <VscAccount  />
                </Link>
              </div>
          ...
          {!username && (
            <>
              ...
              <div className="uppercase space-x-4 right-0">
                <Link className={`"text-zinc-900 hover:text-zinc-700`} to={'register'}>
                  Sign In/ Sign Up
                </Link>
              </div>
            </>
